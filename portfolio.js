...
// Убираем API_BASE через прокси, используем прямые URL
...
// ============================================================
// API FETCH (ПОЛНОСТЬЮ ИЗ СТАРОЙ ВЕРСИИ)
// ============================================================
    ...
    try {
        ...
        try {
            ...
            if (page2Res.ok) {
                ...
                console.warn('⚠️ Page 2 not loaded, status:', page2Res.status);
            }
        } catch (e2) {
        ...
        try {
            const globalRes = await apiFetch(`${COINGECKO_API}/global`);
            if (globalRes.ok) {
        ...
        try {
            const fearRes = await fetch(FEAR_API);
            if (fearRes.ok) {
    ...
    try {
        ...
        for (let i = 0; i < missingFromCache.length; i += chunkSize) {
            ...
            try {
                ...
                console.warn('⚠️ Прямой simple price не удался:', e.message);
            }
            
            // ✅ СПОСОБ 2: Полный запрос через markets (если simple не сработал)
            if (!loaded) {
                try {
                    const url = `${COINGECKO_API}/coins/markets?vs_currency=usd&ids=${chunk.join(',')}&sparkline=true&price_change_percentage=24h,7d,30d`;
                    console.log(`📡 Прямой markets запрос: ${url}`);
                    ...
                    if (res.ok) {
                        ...
                        if (data && data.length > 0) {
                            data.forEach(c => { 
                                extraCoins[c.id] = c; 
                                console.log(`✅ Загружена полная информация: ${c.symbol} (${c.name})`);
                            });
                            loaded = true;
                    ...
                    console.warn('⚠️ Прямой markets запрос не удался:', e.message);
                }
            }
            
            // ✅ СПОСОБ 3: Через прокси (запасной вариант)
            if (!loaded) {
                try {
                    const proxyUrl = `/api/coingecko?path=simple/price?ids=${chunk.join(',')}&vs_currencies=usd&include_market_cap=true&include_24hr_change=true`;
                    ...
                    if (proxyRes.ok) {
                        ...
                        chunk.forEach(id => {
                            if (data[id]) {
                                ...
                                console.log(`✅ Загружена через прокси: ${id} (${data[id].usd})`);
                                loadedCount++;
                            }
                        });
                        loaded = true;
            ...
            // ✅ СПОСОБ 4: Создаем заглушку, если ничего не работает
            if (!loaded) {
...
async function updatePlaceholderPrices() {
    ...
    // Загружаем цены для заглушек через Simple Price API
    const chunkSize = 20;
    let updated = 0;
    ...
    for (let i = 0; i < placeholders.length; i += chunkSize) {
        ...
        try {
            // ⭐ ИСПОЛЬЗУЕМ ПРЯМОЙ ЗАПРОС К COINGECKO (как в старой версии)
            const url = `${COINGECKO_API}/simple/price?ids=${chunk.join(',')}&vs_currencies=usd&include_24hr_change=true`;
            console.log(`📡 Прямой запрос цен: ${url}`);
            ...
            if (res.ok) {
                ...
                console.warn(`⚠️ Прямой запрос не удался, статус: ${res.status}`);
                
                // ⭐ ПРОБУЕМ ЧЕРЕЗ ПРОКСИ КАК ЗАПАСНОЙ ВАРИАНТ
                try {
                    const proxyUrl = `/api/coingecko?path=simple/price?ids=${chunk.join(',')}&vs_currencies=usd&include_24hr_change=true`;
                    console.log(`📡 Запрос через прокси: ${proxyUrl}`);
                    
                    const proxyRes = await fetch(proxyUrl);
                    if (proxyRes.ok) {
                        const prices = await proxyRes.json();
                        chunk.forEach(id => {
                            if (prices[id] && prices[id].usd > 0) {
                                extraCoins[id].current_price = prices[id].usd;
                                extraCoins[id].price_change_percentage_24h = prices[id].usd_24h_change || 0;
                                extraCoins[id].isPlaceholder = false;
                                updated++;
                                console.log(`✅ Обновлена цена через прокси для ${extraCoins[id].symbol}: $${prices[id].usd}`);
                            }
                        });
                    }
                } catch (proxyErr) {
                    console.warn('⚠️ Прокси тоже не сработал:', proxyErr.message);
                }
            }
        } catch (e) {
            console.warn('⚠️ Не удалось обновить цены:', e.message);
        }
...
async function loadCoinChart(coinId, days, attempt) {
    ...
    try {
        const res = await fetch(`/api/coingecko?path=coins/${coinId}/market_chart?vs_currency=usd&days=${days}`);
        
        if (res.status === 429) {
            if (attempt < 3) { 
                await new Promise(r => setTimeout(r, 1500 * attempt)); 
                return loadCoinChart(coinId, days, attempt + 1); 
            }
            if (loading) loading.textContent = 'CoinGecko временно ограничил запросы (429). Подождите минуту.';
            return;
        }
        if (res.status === 404) {
        ...
        if (loading) loading.textContent = 'не удалось загрузить график: ' + (e.message || 'сетевая ошибка');
    }
}
...
function searchCoins(query) {
    ...
    searchDebounce = setTimeout(async () => {
        try {
            // ИСПОЛЬЗУЕМ COINGECKO_API ВМЕСТО API_BASE
            const res = await apiFetch(`${COINGECKO_API}/search?query=${encodeURIComponent(q)}`);
            if (!res.ok) return;
            const data = await res.json();
...
async function fetchExtraCoinFull(id, symbol, image) {
    ...
    try {
        // ИСПОЛЬЗУЕМ COINGECKO_API ВМЕСТО API_BASE
        const res = await apiFetch(`${COINGECKO_API}/coins/markets?vs_currency=usd&ids=${id}&sparkline=true&price_change_percentage=24h,7d,30d`);
        if (res.ok) {
            const data = await res.json();
            if (data && data[0]) {
                extraCoins[id] = data[0];
                try { 
                    const all = JSON.parse(localStorage.getItem('ct_extra_coins') || '{}');
                    all[id] = data[0];
                    localStorage.setItem('ct_extra_coins', JSON.stringify(all)); 
                } catch (e) {}
                return true;
            }
        }
    ...
    try {
        // ИСПОЛЬЗУЕМ COINGECKO_API ВМЕСТО API_BASE
        const res2 = await apiFetch(`${COINGECKO_API}/simple/price?ids=${id}&vs_currencies=usd&include_market_cap=true&include_24hr_change=true`);
...
// ============================================================
// ОБНОВЛЕНИЕ ТОЛЬКО ЦЕН ПОРТФЕЛЯ (легкий запрос)
// ============================================================
...
setInterval(checkNotifs, 10000);
 ...
 // ✅ ПРОВЕРКА ПОРТФЕЛЬНЫХ МОНЕТ
    setTimeout(async function() {
     ...
     // ✅ Обновляем цены заглушек через 5 секунд
    setTimeout(async function() {
